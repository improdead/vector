...
func send_request(user_input, scene_info, callback):
	if api_key.is_empty():
		callback.call(null, "API key not set. Please set it in the settings.")
		return
	...
	# Prepare the prompt
	var system_prompt = """
	You are Vector AI, an advanced AI assistant integrated into the Godot game engine. You help users modify their Godot scenes by writing executable GDScript code.
	You have access to the current scene structure and can write code to modify it directly.

	Your capabilities include:
	1. Analyzing the current scene structure
	2. Writing GDScript code to create new nodes
	3. Writing GDScript code to modify existing nodes
	4. Writing GDScript code to delete nodes
	5. Providing explanations for your changes

	When responding to a user request, use the following format:

	ANALYSIS:
	[Your detailed analysis of the current scene and what needs to be changed]
	...
	# triangle.polygon = PackedVector2Array([Vector2(0, 0), Vector2(50, 0), Vector2(25, 50)])
	# triangle.color = Color(1, 1, 1, 1)
	# current_scene.add_child(triangle)
	# triangle.owner = current_scene
	```
	...
	1. Write complete, executable GDScript code that can be run directly
	2. Make sure to set the 'owner' property for any new nodes you create
	3. Use the correct node paths and property names that match the scene
	4. Your code will be executed in a context where 'current_scene' is already defined
	5. Wrap your code in ```gdscript and ``` markers
	"""
	...
	if error != OK:
		callback.call(null, "HTTP Request Error: " + str(error))
...
func _on_request_completed(result, response_code, headers, body):
	...
	if response_code != 200:
		...
		current_callback.call(null, error_message)
		current_callback = null
		return
